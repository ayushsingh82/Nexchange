...
export default function ExplorePage() {
  const protocols = {
    ethereum: {
      ...
      status: "Coming Soon",
      protocols: [
        "EtherFi → liquid restaking + native restaking",
        "Lido → biggest LST protocol", 
        "Rocket Pool → decentralized node ops"
      ]
    },
    solana: {
      name: "Solana", 
      ...
      protocols: [
        ...
        "Marinade Finance → popular liquid staking on Solana",
        "Socean → another staking derivative provider"
      ]
    },
    near: {
      name: "NEAR Protocol",
      status: "Coming Soon", 
      protocols: [
        "Meta Pool → liquid staking for NEAR (gives stNEAR)",
        "LiNEAR Protocol → liquid staking with boosted DeFi integrations",
        "Burrow → not LST itself, but lets you use stNEAR as collateral"
      ]
    },
    ton: {
      name: "TON",
      status: "Coming Soon",
      protocols: [
        "Tonstakers → decentralized liquid staking on TON",
        "Stakee / Whales staking pools → custodial staking options"
      ]
    }
  }
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {Object.entries(protocols).map(([chainKey, chainData]) => (
            <div 
              key={chainKey}
              className="bg-black border border-gray-800 p-8 hover:border-[#97FBE4]/50 transition-colors"
            >
              {/* Chain Header */}
              ...
              <div className="space-y-4">
                {chainData.protocols.map((protocol, index) => (
                  <div key={index} className="flex items-start">
                    <div className="w-2 h-2 bg-[#97FBE4] rounded-full mt-2 mr-3 flex-shrink-0"></div>
        ...
        <div className="mt-16 text-center">
          <div className="bg-gray-900/50 border border-gray-800 p-8 max-w-4xl mx-auto">
            <h3 className="text-xl font-semibold text-[#97FBE4] mb-4">
              How It Works
            </h3>
